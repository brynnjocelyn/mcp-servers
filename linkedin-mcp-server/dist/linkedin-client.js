...
export class LinkedInClient {
    ...
    async authenticate() {
        ...
        if (!this.config.clientId || !this.config.clientSecret) {
            throw new Error('LinkedIn client_id and client_secret are required for authentication. ' +
     ...
     * Get user's posts
     */
    async getUserPosts(start = 0, count = 10) {
        const userId = await this.getUserId();
     ...
     */
    async searchCompanies(query, start = 0, count = 10) {
     ...
     */
    async shareAsCompany(companyId, params) {
        const requestBody = {
