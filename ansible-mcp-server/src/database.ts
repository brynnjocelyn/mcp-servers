...
export class AnsibleDatabase {
  ...
  async createRun(run: PlaybookRun): Promise<number> {
    const result = await this.run(
      `INSERT INTO playbook_runs (playbook, inventory, start_time, status, tags, limit_hosts, extra_vars, check_mode)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [run.playbook, run.inventory, run.startTime, run.status, run.tags, run.limit, run.extraVars, run.checkMode ? 1 : 0]
    );
    return (result as any).lastID;
  }

  async updateRun(id: number, updates: Partial<PlaybookRun>): Promise<void> {
    const fields = [];
    const values = [];
  ...
  async getRun(id: number): Promise<PlaybookRun | null> {
    const row = await this.get('SELECT * FROM playbook_runs WHERE id = ?', [id]);
    return row ? this.mapRun(row) : null;
  }

  async getRecentRuns(limit: number = 10): Promise<PlaybookRun[]> {
    const rows = await this.all(
      'SELECT * FROM playbook_runs ORDER BY start_time DESC LIMIT ?',
      [limit]
    );
    return rows.map(this.mapRun);
  }

  async getRunsByStatus(status: string): Promise<PlaybookRun[]> {
    const rows = await this.all(
      'SELECT * FROM playbook_runs WHERE status = ? ORDER BY start_time DESC',
      [status]
    );
    return rows.map(this.mapRun);
  }
  ...
  // Inventory methods
  async upsertHost(host: InventoryHost): Promise<void> {
    await this.run(
      `INSERT OR REPLACE INTO inventory_hosts 
  ...
  async getHost(hostname: string): Promise<InventoryHost | null> {
    const row = await this.get('SELECT * FROM inventory_hosts WHERE hostname = ?', [hostname]);
    return row ? this.mapHost(row) : null;
  }

  async getHostsByGroup(groupName: string): Promise<InventoryHost[]> {
    const rows = await this.all(
      'SELECT * FROM inventory_hosts WHERE group_name = ? AND enabled = 1',
      [groupName]
    );
    return rows.map(this.mapHost);
  }

  async getAllHosts(enabled: boolean = true): Promise<InventoryHost[]> {
    const rows = await this.all(
      'SELECT * FROM inventory_hosts WHERE enabled = ? ORDER BY group_name, hostname',
      [enabled ? 1 : 0]
    );
    return rows.map(this.mapHost);
  }

  async getContainers(): Promise<InventoryHost[]> {
    const rows = await this.all(
      'SELECT * FROM inventory_hosts WHERE is_container = 1 AND enabled = 1 ORDER BY proxmox_node, proxmox_vmid',
      []
    );
    return rows.map(this.mapHost);
  }

  private mapHost(row: any): InventoryHost {
    return {
      id: row.id,
  ...
  // Playbook methods
  async upsertPlaybook(playbook: Playbook): Promise<void> {
    await this.run(
  ...
  async getPlaybook(name: string): Promise<Playbook | null> {
    const row = await this.get('SELECT * FROM playbooks WHERE name = ?', [name]);
    return row ? this.mapPlaybook(row) : null;
  }

  async getPlaybooksByCategory(category: string): Promise<Playbook[]> {
    const rows = await this.all(
  ...
  async updatePlaybookStats(name: string, duration: number): Promise<void> {
    await this.run(
      `UPDATE playbooks 
