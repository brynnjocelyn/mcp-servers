...
import { AnsibleDatabase, PlaybookRun } from './database.js';
...
export class AnsibleRunner {
  ...
  async runPlaybook(
    ...
    if (options.becomeUser) args.push('--become-user', options.becomeUser);
    if (options.checkMode) args.push('--check');
    if (options.diff) args.push('--diff');
    if (options.forks) args.push('--forks', options.forks.toString());
    if (options.verbose) args.push(`-${'v'.repeat(Math.min(options.verbose, 4))}`);
    ...
    if (options.startAtTask) args.push('--start-at-task', options.startAtTask);
  ...
  async runCommand(
    ...
    if (moduleArgs) args.push('-a', moduleArgs);
  ...
  async getInventory(options: {
    ...
    if (options.graph) {
      args.push('--graph');
      if (options.vars) args.push('--vars');
    }
   ...
   */
  async checkSyntax(playbookPath: string, inventory?: string): Promise<AnsibleResult> {
   ...
   */
  async installGalaxyRequirements(
    requirementsFile: string,
    type: 'role' | 'collection' = 'role'
  ): Promise<AnsibleResult> {
   ...
   */
  async scanForPlaybooks(directory?: string): Promise<Array<{
    name: string;
    path: string;
    category: string;
    description?: string;
   ...
   */
  async generateInventory(): Promise<any> {
    ...
    for (const host of hosts) {
      ...
      // Add host vars
      const hostVars: any = {};
      if (host.ansibleHost) hostVars.ansible_host = host.ansibleHost;
      if (host.ansiblePort) hostVars.ansible_port = host.ansiblePort;
      if (host.ansibleUser) hostVars.ansible_user = host.ansibleUser;
   ...
   */
  private executeAnsible(command: string, args: string[]): Promise<AnsibleResult> {
