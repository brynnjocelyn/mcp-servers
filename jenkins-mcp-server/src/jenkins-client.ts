...
export class JenkinsClient {
  ...
  private crumbIssuer?: { crumb: string; crumbRequestField: string };
   ...
   */
  async listJobs(folderPath?: string, limit?: number, offset?: number): Promise<{ jobs: any[], total: number, hasMore: boolean }> {
    return this.rateLimiter.execute(async () => {
   ...
   */
  async createOrUpdateJob(jobName: string, configXml: string): Promise<void> {
    try {
   ...
   * Build a job
   */
  async buildJob(jobName: string, parameters?: Record<string, any>, onProgress?: (message: string) => void): Promise<number> {
    if (onProgress) {
   ...
   * Get build log
   */
  async getBuildLog(jobName: string, buildNumber: number, start: number = 0): Promise<string> {
   ...
   */
  async listArtifacts(jobName: string, buildNumber: number): Promise<any[]> {
    const response = await this.client.get(`/job/${jobName}/${buildNumber}/api/json`, {
   ...
   * Download artifact
   */
  async downloadArtifact(jobName: string, buildNumber: number, artifactPath: string): Promise<Buffer> {
    const response = await this.client.get(
      `/job/${jobName}/${buildNumber}/artifact/${artifactPath}`,
   ...
   * Take node offline
   */
  async setNodeOffline(nodeName: string, message: string = ''): Promise<void> {
   ...
   * Restart Jenkins
   */
  async restart(safe: boolean = true): Promise<void> {
   ...
   * Create a view
   */
  async createView(viewName: string, viewType: string = 'ListView'): Promise<void> {
