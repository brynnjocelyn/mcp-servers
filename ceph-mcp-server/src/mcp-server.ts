...
import { CephClient, CephError } from './ceph-client.js';

// Tool schemas
const EmptySchema = z.object({});

const PoolNameSchema = z.object({
  pool_name: z.string().describe('Name of the pool')
});

const OptionalPoolSchema = z.object({
  pool_name: z.string().optional().describe('Name of the pool (optional)')
});

const CreatePoolSchema = z.object({
  name: z.string().describe('Name of the new pool'),
  pg_num: z.number().min(1).default(128).describe('Number of placement groups')
});

const ConfigSchema = z.object({
  section: z.string().optional().describe('Configuration section'),
  name: z.string().optional().describe('Configuration parameter name')
});

const SetConfigSchema = z.object({
  section: z.string().describe('Configuration section'),
...
const RBDImageSchema = z.object({
  name: z.string().describe('Image name'),
  pool: z.string().optional().describe('Pool name (optional)')
});

const CreateRBDImageSchema = z.object({
  name: z.string().describe('Image name'),
  size: z.string().describe('Image size (e.g., 10G, 1T)'),
  pool: z.string().optional().describe('Pool name (optional)')
});

const RGWUserSchema = z.object({
  uid: z.string().describe('User ID'),
  display_name: z.string().describe('Display name')
});

const RGWUserIdSchema = z.object({
  uid: z.string().describe('User ID')
});

const RGWBucketSchema = z.object({
...
async function main() {
  ...
  const cephClient = new CephClient(config);
  ...
  const handleError = (error: any): any => {
    if (error instanceof CephError) {
      throw new McpError(
        ErrorCode.InternalError,
        `Ceph error: ${error.message}`,
        error.details
      );
    }
    throw error;
