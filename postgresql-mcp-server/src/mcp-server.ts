 ...
 */
function formatQueryResult(result: QueryResult): any {
  return {
    rows: result.rows,
    ...
    switch (name) {
      ...
      case 'insert': {
        ...
        let query = format('INSERT INTO %I.%I (%s) VALUES (%s)', 
          schema, 
          table, 
          columns.map(c => format.ident(c)).join(', '),
          placeholders
        );
      ...
      case 'select': {
        ...
        let query = format('SELECT %s FROM %I.%I', 
          columns.map(c => c === '*' ? c : format.ident(c)).join(', '),
          schema,
          table
        );
