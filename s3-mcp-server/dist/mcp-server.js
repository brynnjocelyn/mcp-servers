...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            ...
            // Object operations
            case 'list_objects': {
                const { bucket, prefix, recursive, maxKeys } = args;
            ...
            case 'put_object': {
                const { bucket, object, content, contentType, metadata } = args;
            ...
            case 'copy_object': {
                const { sourceBucket, sourceObject, destBucket, destObject } = args;
            ...
            case 'presigned_get_object': {
                const { bucket, object, expiry } = args;
        ...
        throw new McpError(ErrorCode.InternalError, `S3 operation failed: ${errorMessage}`);
    }
});
/**
