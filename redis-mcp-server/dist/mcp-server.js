...
let redis; // Using any for Cluster type compatibility
let pubClient;
let subClient;
/**
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            ...
            case 'zrange': {
                ...
                const result = withScores
                    ? await redis.zrange(key, min, max, 'WITHSCORES')
            ...
            case 'zrevrange': {
                const { key, min = 0, max = -1, withScores } = SortedSetSchema.parse(args);
                const result = withScores
                    ? await redis.zrevrange(key, min, max, 'WITHSCORES')
                    : await redis.zrevrange(key, min, max);
                if (withScores && Array.isArray(result)) {
                    const formatted = [];
                    for (let i = 0; i < result.length; i += 2) {
                        formatted.push({
            ...
            case 'xread': {
                ...
                if (block !== undefined) {
                    if (count !== undefined) {
                        result = await redis.xread('BLOCK', block, 'COUNT', count, 'STREAMS', ...streamKeys, ...streamIds);
                    }
                    else {
                        result = await redis.xread('BLOCK', block, 'STREAMS', ...streamKeys, ...streamIds);
                    }
                }
            ...
            case 'geodist': {
                ...
                const distance = await redis.geodist(key, member1, member2, unit);
            ...
            case 'georadius': {
                ...
                const options = [];
                if (withCoord)
                    options.push('WITHCOORD');
                if (withDist)
                    options.push('WITHDIST');
                if (count)
            ...
            case 'config_get': {
                ...
                const config = await redis.config('GET', parameter);
        ...
        throw new McpError(ErrorCode.InternalError, `Redis operation failed: ${errorMessage}`);
    }
});
// Start the server
async function main() {
